# 正文 v4 重写批次：chapter-0001 至 chapter-0100

## 批次原则

- `chapter-0001` 与 `chapter-0002` 作为精写样章处理。
- `chapter-0003` 至 `chapter-0100` 以旧正文为素材底稿，嵌入 detailed-v4 的本章交易、代价和拒绝后果。
- `chapter-0001` 至 `chapter-0025` 已完成第一轮章群精修，后续仍可做诗性与句级定稿。
- `chapter-0026` 至 `chapter-0100` 仍是结构合格初稿，后续应按每 25 章人工通读一次，消除批处理痕迹。

## 章节清单

| 章节 | 状态 | 处理方式 |
|---|---|---|
| chapter-0001 | v4-rewrite-polished | 精写重写 |
| chapter-0002 | v4-rewrite-polished | 精写重写 |
| chapter-0003 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0004 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0005 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0006 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0007 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0008 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0009 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0010 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0011 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0012 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0013 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0014 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0015 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0016 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0017 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0018 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0019 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0020 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0021 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0022 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0023 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0024 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0025 | v4-rewrite-polished | 章群精修：清理模板句，补强证物/交易/代价/钩子 |
| chapter-0026 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0027 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0028 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0029 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0030 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0031 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0032 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0033 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0034 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0035 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0036 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0037 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0038 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0039 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0040 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0041 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0042 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0043 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0044 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0045 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0046 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0047 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0048 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0049 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0050 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0051 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0052 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0053 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0054 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0055 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0056 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0057 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0058 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0059 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0060 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0061 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0062 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0063 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0064 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0065 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0066 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0067 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0068 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0069 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0070 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0071 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0072 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0073 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0074 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0075 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0076 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0077 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0078 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0079 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0080 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0081 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0082 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0083 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0084 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0085 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0086 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0087 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0088 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0089 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0090 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0091 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0092 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0093 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0094 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0095 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0096 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0097 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0098 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0099 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
| chapter-0100 | v4-rewrite-draft | 旧正文素材重组 + 本章交易嵌入 |
