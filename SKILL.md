...
11. **长篇自动续写到目标字数**：当用户要求“写到目标字数”“一口气写完”“不要一章一章问我”时，进入自动连续写作模式。后续每轮都按项目状态连续推进最早 `planned` 章节，尽量写满完整章节、审稿、同步、提交；不能用提纲或占位符冒充正文，也不能把未写内容计入总字数。
...
| 自动连续写作模式 | `novel-writing` → `novel-draft` → `novel-review` → `novel-update` | 用户要求写到目标字数、不再逐章确认；连续推进完整章节直到目标字数或上下文上限 |
...
| 写到目标字数、一口气写完、不要逐章询问 | 原生连续写作优先；需要工具化批量时再进入 `skills/inkos-bridge`，但所有输出必须回本项目复审 |
